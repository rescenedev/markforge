                    ...
                    for (child_ix, child) in children.iter().enumerate() {
                        match child {
                            BlockNode::Paragraph { .. } => {
                                ...
                                // Size the task-list checkbox relative to the
                                // actual text size (not `rem`), and vertically
                                // center it within the first line, so it stays
                                // aligned at any font size / zoom.
                                let cb_text =
                                    window.text_style().font_size.to_pixels(window.rem_size());
                                let cb_box = cb_text * 0.875;
                                let cb_line = window.line_height();
                                let cb_mt = if cb_line > cb_box {
                                    (cb_line - cb_box) * 0.5
                                } else {
                                    cb_box * 0.
                                };
                                let cb_icon = cb_box * 0.7;
                                                ...
                                                div()
                                                    ...
                                                    .when(checked, |this| {
                                                        this.bg(cx.theme().primary).child(
                                                            Icon::new(IconName::Check).size(cb_icon),
                                                        )
                                                    }),
                                            )
                                        })
